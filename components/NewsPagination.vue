    ...
    <div class="center-align" v-if="pages <= 6">
      <ul class="pagination">
        <li v-bind:class="getLeftChevronClass">
          <nuxt-link :to="`/news/${previousPage}`"><i class="material-icons">chevron_left</i></nuxt-link>
        </li>
        <li v-for="page in pages" :key="page" v-bind:class="getPageClass(page)">
          <nuxt-link :to="`/news/${page}`">{{ page }}</nuxt-link>
        </li>
        <li v-bind:class="getRightChevronClass">
          <nuxt-link :to="`/news/${nextPage}`"><i class="material-icons">chevron_right</i></nuxt-link>
        </li>
      </ul>
    </div>
    ...
    <div class="center-align" v-if="pages > 6">
      ...
      <ul class="pagination" v-else-if="currentPage === 3">
        ...
        <li v-bind:class="getPageClass(page)">
          <nuxt-link :to="`/news/${pages}`">{{ pages }}</nuxt-link>
        </li>
        <li v-bind:class="getRightChevronClass">
          <nuxt-link :to="`/news/${nextPage}`"><i class="material-icons">chevron_right</i></nuxt-link>
        </li>
      ...
      <ul class="pagination" v-else-if="currentPage === pages - 2">
        ...
        <li v-bind:class="getPageClass(page)">
          <nuxt-link :to="`/news/1`">1</nuxt-link>
        </li>
        <span>...</span>
...
export default {
  computed: {
    pages() {
      return this.$store.state.newsPages;
    },
    currentPage() {
